...
- `medias.ts`: Media management table
...
- `media_id`: Foreign key to medias table (optional)
- `created_at`: Creation timestamp
- `deleted_at`: Soft delete timestamp
...
- `id`: Primary key
- `file`: Base64 encoded image file or URL
...
- `media_id`: Foreign key to medias table
- `group_id`: Foreign key to groups table (optional)
- `user_id`: Foreign key to users table
